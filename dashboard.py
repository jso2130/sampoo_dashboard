...
# ─── 계면활성제별 화학 파생변수 기여 계수 (배합비 1%당) ──────────────────────
# 점도/탁도 모델 피처: C[N+]_derived, CS=O_derived, CC=O[O-]_derived,
...
def build_features(s1, v1, s2, v2, poly, poly_v, thick, thick_v):
    ...
    return {
        ...
        'quat_d': quat_d, 'sulfo_d': sulfo_d,
        'carb_d': carb_d, 'acetal_d': acetal_d,
...
def predict_viscosity(f):
    # 점도예측모델.py 마지막 XGBoost (이진분류: HIGH+MEDIUM vs VERY-LOW)
    # 주요 피처: Arlypon TT/F (증점제), 폴리머 농도, CC(=O)[O-]_derived
    score = (f['Arlypon TT'] * 1.20 +
             f['Arlypon F']  * 1.00 +
             f['Salcare Super 7']    * 0.60 +
             f['Luviquat Excellence']* 0.45 +
             f['Dehyquart CC6']      * 0.40 +
             f['Dehyquart CC7 Benz'] * 0.40 +
             f['carb_d']             * 0.50)
    if score >= 1.5:
        return '높음 / 보통'
    else:
...
def predict_turbidity(f):
    # 탁도예측모델.py LightGBM + Optuna (이진분류: 맑음 vs 탁함)
    # SHAP 상위 피처: CC(=O)[O-]_derived, C_neg_Total_Ratio → 탁함 기여
    #                [H]C(C)(OC)OC_derived (APG계) → 맑음 기여
    clarity = (f['acetal_d']          * 2.50 +
               f['Salcare Super 7']   * 0.80 +
               f['quat_d']            * 1.20 -
               f['carb_d']            * 1.80 -
               f['C_neg_Total_Ratio'] * 1.50 +
               f['C_pos_Total_Ratio'] * 0.80)
...
with left:
    st.markdown(
        f"""
        <div style='border: 2px solid #ccc; border-radius: 12px; padding: 32px; text-align: center;'>
            <p style='font-size: 20px; font-weight: 600; margin-bottom: 12px;'>점도 등급</p>
            <p style='font-size: 60px; font-weight: bold; margin: 0;'>{visc_label}</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
with right:
    st.markdown(
        f"""
        <div style='border: 2px solid #ccc; border-radius: 12px; padding: 32px; text-align: center;'>
            <p style='font-size: 20px; font-weight: 600; margin-bottom: 12px;'>탁도 등급</p>
            <p style='font-size: 60px; font-weight: bold; margin: 0;'>{turb_label}</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
